...
struct FDynamicUniformBufferObject
{
...
FVulkanMeshRenderer::~FVulkanMeshRenderer()
{
	...
	for (const auto& Pair : DrawingInfos)
	{
		const FDrawingInfo& DrawingInfo = Pair.second;

		for (const FVulkanBuffer& DynamicUniformBuffer : DrawingInfo.DynamicUniformBuffers)
		{
			vkFreeMemory(Device, DynamicUniformBuffer.Memory, nullptr);
			vkDestroyBuffer(Device, DynamicUniformBuffer.Buffer, nullptr);
		}
	}
...
void FVulkanMeshRenderer::GatherDrawingInfo()
{
	...
	for (FVulkanModel* Model : Scene->GetModels())
	{
		...
		auto Iter = DrawingInfos.find(Mesh);
		if (Iter == DrawingInfos.end())
		{
			Iter = DrawingInfos.insert({ Mesh, {} }).first;
		}
...
void FVulkanMeshRenderer::CreateUniformBuffers()
{
	...
	uint32_t UniformBufferSize = sizeof(FUniformBufferObject);
	...
	for (auto& Pair : DrawingInfos)
	{
		...
		FDrawingInfo& DrawingInfo = Pair.second;

		uint32_t DynamicAlignment = sizeof(FDynamicUniformBufferObject);
		if (MinAlignment > 0)
		{
		...
		for (size_t Idx = 0; Idx < MAX_CONCURRENT_FRAME; ++Idx)
		{
			Vk::CreateBuffer(
				...
				VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
				VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
				DrawingInfo.DynamicUniformBuffers[Idx].Buffer,
				DrawingInfo.DynamicUniformBuffers[Idx].Memory);

			vkMapMemory(Device, DrawingInfo.DynamicUniformBuffers[Idx].Memory, 0, UniformBufferSize, 0, &DrawingInfo.DynamicUniformBuffers[Idx].Mapped);
		}
	}
}
...
void FVulkanMeshRenderer::CreateDescriptorSets()
{
	...
	for (auto& Pair : DrawingInfos)
	{
		...
		DrawingInfo.DescriptorSets.resize(MAX_CONCURRENT_FRAME);
		VK_ASSERT(vkAllocateDescriptorSets(Device, &DescriptorSetAllocInfo, DrawingInfo.DescriptorSets.data()));
	}
...
void FVulkanMeshRenderer::UpdateUniformBuffer()
{
	...
	UBO.Shininess = Light.Shininess;

	memcpy(UniformBuffers[Context->GetCurrentFrame()].Mapped, &UBO, sizeof(FUniformBufferObject));
}

void FVulkanMeshRenderer::UpdateDynamicUniformBuffer(FVulkanMesh* InMesh)
{
	if (InMesh == nullptr)
	{
		return;
	...
	auto Iter = DrawingInfos.find(InMesh);
	if (Iter == DrawingInfos.end())
	{
		return;
	}

	FDrawingInfo& DrawingInfo = Iter->second;

	static const glm::mat4 IdentityMatrix(1.0f);

	std::vector<FDynamicUniformBufferObject> DynamicUBOData;
	for (int Idx = 0; Idx < DrawingInfo.Models.size(); ++Idx)
	{
		FVulkanModel* Model = DrawingInfo.Models[Idx];
		if (Model == nullptr)
		{
			continue;
		}
		...
		FDynamicUniformBufferObject DynamicUBO{};
		DynamicUBO.Model = glm::translate(IdentityMatrix, ModelTransform.GetTranslation()) * glm::toMat4(ModelTransform.GetRotation()) * glm::scale(IdentityMatrix, ModelTransform.GetScale());

		DynamicUBOData.push_back(DynamicUBO);
	}

	memcpy(DrawingInfo.DynamicUniformBuffers[Context->GetCurrentFrame()].Mapped, DynamicUBOData.data(), DynamicUBOData.size() * sizeof(FDynamicUniformBufferObject));

	VkDevice Device = Context->GetDevice();

	VkMappedMemoryRange MemoryRange{};
	MemoryRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
	MemoryRange.memory = DrawingInfo.DynamicUniformBuffers[Context->GetCurrentFrame()].Memory;
	MemoryRange.size = DynamicUBOData.size() * sizeof(FDynamicUniformBufferObject);
...
void FVulkanMeshRenderer::UpdateDescriptorSets()
{
	...
	for (auto& Pair : DrawingInfos)
	{
		...
		const std::vector<VkDescriptorSet>& DescriptorSets = DrawingInfo.DescriptorSets;
		...
		for (int32_t Idx = 0; Idx < DescriptorSets.size(); ++Idx)
		{
			...
			DynamicBufferInfo.buffer = DrawingInfo.DynamicUniformBuffers[Idx].Buffer;
			DynamicBufferInfo.offset = 0;
			DynamicBufferInfo.range = sizeof(FDynamicUniformBufferObject);
...
void FVulkanMeshRenderer::Render()
{
	...
	for (const auto& Pair : DrawingInfos)
	{
		...
		UpdateDynamicUniformBuffer(Mesh);

		const FDrawingInfo& DrawingInfo = Pair.second;
		VkDescriptorSet DescriptorSet = DrawingInfo.DescriptorSets[Context->GetCurrentFrame()];

		uint32_t DynamicIndex = 0;

		for (FVulkanModel* Model : DrawingInfo.Models)
		{
			if (Model == nullptr)
			{
				continue;
