...
{
	GWindow = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "VkCamera", nullptr, nullptr);
	if (GWindow == nullptr)
	{
...
void CreateUniformBuffers()
{
	...
	{
		CreateBuffer(UniformBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, GUniformBuffers[Idx].Buffer, GUniformBuffers[Idx].Memory);
...
{
	float FOVRadians = glm::radians(GCamera.FOV);
	float AspectRatio = GSwapchainExtent.width / (float)GSwapchainExtent.height;

	glm::quat CameraQuaternion(GCamera.Rotation);
	glm::mat4 RotationMatrix = glm::toMat4(CameraQuaternion);
	glm::mat4 TranslationMatrix = glm::translate(glm::mat4(1.0f), -GCamera.Position);

	FUniformBufferObject UBO{};
	UBO.Model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -2.0f));
	UBO.View = RotationMatrix * TranslationMatrix;
